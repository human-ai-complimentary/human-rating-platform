...
def _upload_questions(client: TestClient, experiment_id: int) -> None:
    ...
    response = client.post(
        f"/admin/experiments/{experiment_id}/upload",
...
def test_upload_questions_records_upload_and_stats(client: TestClient):
    ...
    uploads_response = client.get(f"/admin/experiments/{experiment_id}/uploads")
    stats_response = client.get(f"/admin/experiments/{experiment_id}/stats")
...
def test_upload_rejects_non_csv_file(client: TestClient):
    ...
    response = client.post(
        f"/admin/experiments/{experiment['id']}/upload",
...
def test_export_ratings_streams_large_dataset_in_chunks(client: TestClient, sync_engine):
    ...
    with client.stream("GET", f"/admin/experiments/{experiment['id']}/export") as response:
        assert response.status_code == 200
